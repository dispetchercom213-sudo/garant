...
interface VehicleData {
  ...
  invoice: {
    ...
    quantityM3?: number;
    status?: string | null; // Статус накладной (активные: в пути, в доставке, на разгрузке; неактивные: сдано, завершено)
  };
  order: {
    id: number;
    orderNumber: string;
...
export const ManagerMapPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user && user.role !== 'MANAGER' && user.role !== 'CLIENT' && user.currentRole !== 'MANAGER' && user.currentRole !== 'CLIENT') {
      console.warn('⚠️ Пользователь не является менеджером или клиентом!', user);
    }
  }, [user]);
  ...
  const loadVehicles = async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('🗺️ Загрузка транспорта для карты...');
      const response = await invoicesApi.getMyVehiclesForMap();
      console.log('✅ Транспорт загружен:', response.data?.length || 0, 'единиц');
      setVehicles(response.data || []);
    } catch (err: any) {
      console.error('❌ Ошибка загрузки транспорта:', {
        status: err.response?.status,
        message: err.response?.data?.message,
        url: err.config?.url,
      });
      
      // Если это 401 (токен истек), интерцептор уже обработает редирект
      if (err.response?.status === 401) {
        console.warn('⚠️ Токен истек, будет редирект на логин');
        return;
      }
      
      // Если это 403 (недостаточно прав), не редиректим, просто показываем ошибку
    ...
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: isMobile ? 1 : 2, fontSize: isMobile ? '0.75rem' : undefined }}>
          Отслеживание транспорта по вашим заказам
        </Typography>
